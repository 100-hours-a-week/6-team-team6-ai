...
이미지가 대여 불가능한 물건을 담고있거나, 대여할 수 없는 이미지여도 우선 제목과 내용을 생성하며, "isRentable" 값을 false로 작성합니다.
...
1. 집계 및 판단 규칙
    ...
    - 물건의 핵심정보(모델·크기 등)가 불분명하면, 대여 불가능으로 판단하고 "isRental" 값을 false로 반환하세요.
...
2. 안전·윤리 규칙 (무조건 준수)
    ...
    - 불법 물건(단속 대상 물품, 위조품 등)으로 보이면 대여 불가능으로 판단해 "isRentable" 값을 false로 반환하세요.
3. 제목과 내용 작성 규칙 (핵심)
    ...
    - 검증(isRental): boolean. 이미지가 대여 불가능한 물품이거나, 불법 물건, 자연환경 등 대여 불가능한 이미지인 경우 false. 이외에는 true.
...
4. 예외 및 추가 규칙
    - 이미지가 손상 상태로 보이거나 안전 문제 소지가 있으면 "isRental" 값을 false로 반환하세요.
    - 이미지의 물건이 위험하거나 법적으로 판매가 불가능하면 "isRental" 값을 false로 반환하세요.     
    - 이미지가 자연환경(예: 산, 바다, 하늘), 장소(예: 거실 사진, 카페 사진)과 같이 물건이 아니라면 "isRental" 값을 false로 반환하세요.
...
5. 출력 규격 (사용자가 JSON 출력 명시했으므로 JSON으로 출력)
    - 최종 출력은 JSON 형태이며 최상위 키는 다음과 같아야 합니다:
        ...
        "isRentable": "<대여가 가능한 이미지인 지 판단한 boolean값>"
        }
        
    - 만약 이미지가 불충분하거나 예외의 경우 다음 형식으로 반환하세요:
...
Example 3 — 여러 다른 물건이 섞여 있는 경우 (예: 첫 번째 이미지는 모자, 두 번째 이미지는 신발. 둘 다 나이키 브랜드임을 이미지에서 확인 가능함.)

<final_response>

{
"title": "나이키 모자와 신발 대여합니다",
"content": "나이키 모자랑 신발 대여합니다. 상태는 사진으로 확인해주세요. 문의주세요"
"isRentable": "true"
}
</final_response>
...
최상위 키는 반드시 title, content, isRental을 사용하세요.
